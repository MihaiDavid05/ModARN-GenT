...
def read_admissions_table(mimic4_path):
    admits = dataframe_from_csv(os.path.join(mimic4_path, 'core/admissions.csv', compression='infer'))#.gz, no infer
...
def read_patients_table(mimic4_path):
    pats = dataframe_from_csv(os.path.join(mimic4_path, 'core/patients.csv', comprssion='infer'))#.gz, no infer
...
def read_diagnoses_icd_table(mimic4_path):
    diag = dataframe_from_csv(os.path.join(mimic4_path, 'hosp/diagnoses_icd.csv', compression='infer'))#.gz, no infer
    diag.reset_index(inplace=True)
    return diag


def read_d_icd_diagnoses_table(mimic4_path):
    d_icd = dataframe_from_csv(os.path.join(mimic4_path, 'hosp/d_icd_diagnoses.csv', compression='infer'))#.gz, no infer
...
def read_procedures_icd_table(mimic4_path):
    proc = dataframe_from_csv(os.path.join(mimic4_path, 'hosp/procedures_icd.csv', compression='infer'))#.gz, no infer
    proc.reset_index(inplace=True)
    return proc


def read_d_icd_procedures_table(mimic4_path):
    p_icd = dataframe_from_csv(os.path.join(mimic4_path, 'hosp/d_icd_procedures.csv', compression='infer'))#.gz, no infer
...
def preproc_meds(module_path:str, adm_cohort_path:str) -> pd.DataFrame:
    ...
    med = pd.read_csv(module_path, usecols=['subject_id', 'stay_id', 'itemid', 'starttime', 'endtime','rate','amount','orderid'], parse_dates = ['starttime', 'endtime'])
    #  compression='gzip'
...
def preproc_out(dataset_path: str, cohort_path:str, time_col:str, dtypes: dict, usecols: list) -> pd.DataFrame:
    ...
    def merge_module_cohort() -> pd.DataFrame:
        ...
        # read module w/ custom params
        module = pd.read_csv(dataset_path, usecols=usecols, dtype=dtypes, parse_dates=[time_col]).drop_duplicates()
        # compression='gzip'
...
def preproc_chart(dataset_path: str, cohort_path:str, time_col:str, dtypes: dict, usecols: list) -> pd.DataFrame:
    ...
    nstay=[]
    nrows=0
    for chunk in tqdm(pd.read_csv(dataset_path, usecols=usecols, dtype=dtypes, parse_dates=[time_col],chunksize=chunksize)):  # compression='gzip'
        #print(chunk.head())
        count=count+1
...
def preproc_icd_module(module_path:str, adm_cohort_path:str, icd_map_path=None, map_code_colname=None, only_icd10=True) -> pd.DataFrame:
    ...
    def get_module_cohort(module_path:str, cohort_path:str):
        module = pd.read_csv(module_path, header=0)
        # compression='gzip'
        adm_cohort = pd.read_csv(adm_cohort_path, header=0)
        #  compression='gzip'
