...
def feature_icu(cohort_output, version_path, diag_flag=True,out_flag=True,chart_flag=True,proc_flag=True,med_flag=True):
    if diag_flag:
        ...
        diag = preproc_icd_module("./"+version_path+"/hosp/diagnoses_icd.csv", './data/cohort/'+cohort_output+'.csv.gz', './utils/mappings/ICD9_to_ICD10_mapping.txt', map_code_colname='diagnosis_code')
    ...
    if out_flag:  
        ...
        out = preproc_out("./"+version_path+"/icu/outputevents.csv", './data/cohort/'+cohort_output+'.csv.gz', 'charttime', dtypes=None, usecols=None)
        # outputevents.csv.gz
    ...
    if chart_flag:
        ...
        chart=preproc_chart("./"+version_path+"/icu/chartevents.csv", './data/cohort/'+cohort_output+'.csv.gz', 'charttime', dtypes=None, usecols=['stay_id','charttime','itemid','valuenum','valueuom'])
        # chartevents.csv.gz
    ...
    if proc_flag:
        ...
        proc = preproc_proc("./"+version_path+"/icu/procedureevents.csv", './data/cohort/'+cohort_output+'.csv.gz', 'starttime', dtypes=None, usecols=['stay_id','starttime','itemid'])
    ...
    if med_flag:
        ...
        med = preproc_meds("./"+version_path+"/icu/inputevents.csv", './data/cohort/'+cohort_output+'.csv.gz')
        # inputevents.csv.gz
