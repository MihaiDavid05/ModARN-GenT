...
def get_visit_pts(mimic4_path:str, group_col:str, visit_col:str, admit_col:str, disch_col:str, adm_visit_col:str, use_mort:bool, use_los:bool, los:int, use_admn:bool, disease_label:str,use_ICU:bool):
    ...
    if use_ICU:
        visit = pd.read_csv(mimic4_path + "icu/icustays.csv", header=0, index_col=None, parse_dates=[admit_col, disch_col])
        # compression='gzip'
        if use_admn:
            ...
            pts = pd.read_csv(mimic4_path + "core/patients.csv", header=0, index_col=None, usecols=['subject_id', 'dod'], parse_dates=['dod'])
            # compression='gzip'
        ...
        visit = pd.read_csv(mimic4_path + "core/admissions.csv", header=0, index_col=None, parse_dates=[admit_col, disch_col])
        # compression='gzip'
    ...
    pts = pd.read_csv(
            mimic4_path + "core/patients.csv", header=0, index_col = None, usecols=[group_col, 'anchor_year', 'anchor_age', 'anchor_year_group', 'dod','gender']
        )
    # compression='gzip'
...
#     visit_pts = visit_pts.loc[visit_pts['Age'] >= 18]
    ...
    ##Add Demo data
    eth = pd.read_csv(mimic4_path + "core/admissions.csv", header=0, usecols=['hadm_id', 'insurance','ethnicity'], index_col=None)
    # compression='gzip'
