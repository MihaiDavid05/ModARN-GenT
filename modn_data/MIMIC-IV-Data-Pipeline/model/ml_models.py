...
class ML_models():
    def __init__(self,data_icu,k_fold,model_type,concat,oversampling, undersampling, train=True, own_data=None, encode_cat=True):
        self.train=train
        ...
        if self.k_fold != 0 and not self.train:
            raise ValueError("Cannot only gather (NOT TRAIN) all data if k-folds > 0 !")
        self.ml_train()
    ...
    def create_kfolds(self):
        ...
        if self.undersampling:
            ...
            hids=np.asarray(hids).reshape(-1,1)
            hids, y = undersample.fit_resample(hids, y)
            hids=hids[:,0]
            print("Total Samples",len(hids))
            print("Positive Samples",y.sum())
    ...
    def ml_train(self):
        if self.own_data is not None:
            if self.train:
                X_train,Y_train,X_test,Y_test = self.own_data
                self.test_data=X_test.copy(deep=True)
                self.train_model(X_train,Y_train,X_test,Y_test)
            else:
                print("You just set your data, NO training is happening!")
        else:
            ...
            for i in range(self.k_fold):
                ...
                if self.train:
                    ...
                    print("No training, only data formatting and gathering!")
                    if self.k_fold == 1:
                        print("X_train, X_test, Y_train, Y_test stored as attributes of ml object!")
                        self.X_train = X_train
                        self.Y_train = Y_train
                        self.X_test = X_test
                        self.Y_test = Y_test
                    else:
                        print("Cannot only gather all data if k-folds > 0 !")
    ...
    def train_model(self,X_train,Y_train,X_test,Y_test):
        ...
        if self.model_type=='Gradient Bossting':
            ...
            results = self.loss(prob[:,1],np.asarray(Y_test),logits,False,True)
            self.auc, self.precision, self.recall = results[1], results[5], results[6]
            ...
            prob=model.predict_proba(X_test)
            results = self.loss(prob[:,1],np.asarray(Y_test),logits[:,1],False,True)
            self.auc, self.precision, self.recall = results[1], results[5], results[6]
        ...
        for sample in tqdm.tqdm(ids):
            ...
            if X_df.empty:
                ...
                if len(stat) == 0:
                    ...
                    dyn_stat = pd.concat([dyn_df,stat],axis=1)
                X_df=pd.concat([X_df,pd.concat([dyn_stat,demo],axis=1)],axis=0)
            if y_df.empty:
                y_df=y
            else:
...
#             if X_df.empty:
#                 new_stat = pd.DataFrame(np.repeat(stat.values, len(dyn_df), axis=0))
#                 new_stat.columns = stat.columns
#                 X_df = pd.concat([dyn_df,new_stat],axis=1)

#                 new_demo = pd.DataFrame(np.repeat(demo.values, len(X_df), axis=0))
#                 new_demo.columns = demo.columns
#                 X_df=pd.concat([X_df, new_demo],axis=1)
#             else:
#                 new_stat = pd.DataFrame(np.repeat(stat.values, len(dyn_df), axis=0))
#                 new_stat.columns = stat.columns
#                 dyn_stat_df = pd.concat([dyn_df,new_stat],axis=1)

#                 new_demo = pd.DataFrame(np.repeat(demo.values, len(dyn_stat_df), axis=0))
#                 new_demo.columns = demo.columns
#                 demo_dyn_stat_df = pd.concat([dyn_stat_df,new_demo],axis=1)

#                 X_df=pd.concat([X_df, demo_dyn_stat_df],axis=0)
#             if y_df.empty:
#                 y_df=y
#             else:
#                 y_df=pd.concat([y_df,y],axis=0)
